			...
			<legend>Details</legend>
				<?php 
					if ($content = mysql_fetch_array($result)) {
						...
						//
						// GET /content-6/index.php?id=935137890000 HTTP/1.1
						// Host: localhost
						...
						//
						//
						// (mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))['idusers'] == '</b></fieldset></div></div><script>alert(1)</script>')
						//
						// mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))
						...
						//
						//
						// (mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))['name'] == '</b></fieldset></div></div><script>alert(1)</script>')
						//
						// mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))
						// mysql_select_db(NULL, True)
						...
						//
						//
						// (mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))['email'] == '</b></fieldset></div></div><script>alert(1)</script>')
						//
						// mysql_fetch_array(mysql_query((('SELECT * FROM users WHERE idusers=' . base64_decode($_GET['id'], True)) . ' LIMIT 1')))
						// mysql_select_db(NULL, True)
